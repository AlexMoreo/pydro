...
"""
def fisher_score_binary(feat, c, not_c=None, npos=None, nneg=None):
    if not_c is None:
        not_c = np.ones_like(c) - c
    ...
    muneg = np.mean(feat[not_c == 1])
    mu = np.mean(feat)

    stdpos = np.std(feat[c == 1])
    stdneg = np.std(feat[not_c == 1])
...
def fisher_score_matrix(X, Y):
    fisher_columns = Parallel(n_jobs=-1, backend="threading")(
        delayed(fisher_score_parallel)(X, cat) for cat in Y.T)
    fisher_matrix = np.vstack(fisher_columns).T
    return fisher_matrix


def fisher_score_parallel(X, cat):
    nF = X.shape[1]
    fisher = np.zeros(nF)
    cat_array = cat.toarray()
    cat_neg = np.ones_like(cat_array) - cat_array
    npos = cat.sum()
    nneg = np.sum(cat_neg)
    for f, feat in enumerate(X.T):
        score = fisher_score_binary(feat, cat, not_c=cat_neg, npos=npos, nneg=nneg)
        fisher[f] = score
    print('[done]')
    return fisher